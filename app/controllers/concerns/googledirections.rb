...
#
  ...
  # My revised method that doesn't need 'extlib/hash'
  def initialize(origin, destination)
    origin = origin
    destination = destination
    @url = @@base_url + '?mode=driving&language=en&alternative=true&sensor=false&' + "origin=" + origin + "&destination=" + destination
    @xml = open(@url).read
    @doc = Nokogiri::XML(@xml)
    @status = @doc.css('status').text
  end
